...
def get_task_duration_minutes(task: Task) -> int:
    ...
    if unit in ["minute", "minutes"]:
        ...
        raise ValueError(
            f"Unsupported duration unit '{unit}' for task '{task.content}'"
        )
...
def get_task_due_date(task: Task) -> datetime | None:
    ...
    try:
        ...
        raise ValueError(
            f"Invalid due date format '{task.due.date}' for task '{task.content}'"
        )
...
def get_task_deadline_date(task: Task) -> date | None:
    ...
    try:
        return cast(date, task.deadline.date)
    except ValueError:
        raise ValueError(
            f"Invalid deadline date format '{task.deadline.date}' for task '{task.content}'"
        )
